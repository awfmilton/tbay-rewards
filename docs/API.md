...
| Spend intent lapsed inside 30 days | Erased, with `from_address` replaced by a digest **keyed with `TOKEN_SECRET`** — deliberately not the tenant's `pii_salt`, which sits in the same database as the wallet table it would be matched against. `expires_at` bounds the quote, not the money: a customer who sent their TBAY and lost the tab has tokens at the retailer's payout wallet. The digest answers the only question a settlement asks — "did this transfer come from the wallet this intent was opened for?" — so `POST .../verify` still works, while the address itself is gone and cannot be joined back to anyone. |
| Spend intent `verified`, `cancelled`, or lapsed over 30 days ago | Fully scrubbed. Nothing is owed. |
| Bridge withdrawal, any state | Links nulled and both addresses replaced by a **keyed commitment** (`erased:<tenant>:<digest>`), with an unsettled one counted in `obligations_kept`. The obligation survives: `burn_tx_hash` stays, and `GET /v1/bridge/withdrawals/{id}` returns `payable_to` for a caller presenting the bridge operator credential — the candidate read back from the burn transaction, **checked against the commitment**. A commitment rather than a bare deletion because the recovery would otherwise trust whichever transfer the node returned first: a transaction carrying two burns paid the wrong wallet, and a wrong or hostile node could redirect the payout. A commitment rather than a sentinel address because a well-formed `0x…ff` in a listing is something an operator can copy into an irreversible transfer. `payable_to` is operator-only: the L1 release needs that credential anyway, and returning an erased person's wallet to the retailer on request would undo the erasure. When the chain cannot answer it is `{ address: null, reason }`, never an error. |
